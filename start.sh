...
check_disk_space() {
  while true; do
    ...
    header="🖥️ <b>Сервер:</b> <code>$SERVER_NAME</code>"

    if [ "$disk_usage" -ge 100 ]; then
      send_telegram "$header

❌ <b>ДИСК ЗАПОЛНЕН НА 100%</b>! Требуется немедленное вмешательство!"
    elif [ "$disk_usage" -ge 98 ]; then
      send_telegram "$header

🚨 <b>Диск почти заполнен:</b> ${disk_usage}%! Проверьте, освободите место."
    elif [ "$disk_usage" -ge 96 ]; then
      send_telegram "$header

⚠️ <b>Предупреждение:</b> диск заполнен на ${disk_usage}%. Задумайтесь о том, чтобы освободить место."
    fi
...
❌ <b>ОЗУ почти полностью занята</b> (${mem_usage_percent}%). Требуется немедленная проверка!"
    elif [ "$mem_usage_percent" -ge 95 ]; then
      send_telegram "$header

🚨 <b>Высокое потребление памяти:</b> ${mem_usage_percent}%. Рассмотрите возможность оптимизации."
    elif [ "$mem_usage_percent" -ge 85 ]; then
      send_telegram "$header

⚠️ <b>Использование памяти превышает 85%</b> (${mem_usage_percent}%)."
    fi
...
start_disk_monitoring() {
  if [[ -f /tmp/check_disk_space.pid && -d /proc/$(cat /tmp/check_disk_space.pid) ]]; then
    ...
    header="<b>✅ Мониторинг ресурсов запущен</b>
